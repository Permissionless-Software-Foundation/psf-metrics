...
try {
  var walletInfo = {
    "mnemonic": "gospel prepare layer moment exact good twelve fashion wrestle illegal require artefact",
    "cashAddress": "bchtest:qrtddel54p4zxmrkf7jyex7j06lhx48k3s5wqpgu5p",
    "legacyAddress": "n16vM3Ndnpp43GzDnxhHgshMCXspkvsJoq",
    "WIF": "cTmaWzXyzbbxtvbe6cUFhRxMgSmrVAWo6FC5pk2YMYCURzSZrZjQ"
  }
  //var walletInfo = require('wallet.json')
} catch (err) {
  console.log('Could not open wallet.json. Generate a wallet with create-wallet first.'); throw err;
}
...
class BFP {
  constructor(config) {
    //try { I removed the TRY/CATCH because it makes no sense here as it can not cause any error (untestable), do you agree?
    this.bfp = bfp
    this.bchjs = bchjs
    _this = this
    /*} catch (err) {
