...
class BFP {
  constructor(config) {
    ...
    _this.throwErrorOnInvalidConfigParameters(config)
    ...
    sendersAddress = _this.generateCashAddressFromWif(wif)
    // const SEND_MNEMONIC = walletInfo.mnemonic
  }

  generateCashAddressFromWif(wif) {
  ...
  throwErrorOnInvalidConfigParameters(config) {
    if (config === 'undefined' || config == null) {
      _this.throwMissingPropertyError('config');
    }
    if (!config.wif) {
      _this.throwMissingPropertyError('config.wif');
    }
    if (!config.fileName) {
      _this.throwMissingPropertyError('config.fileName');
    }
    if (!config.fileExt) {
      _this.throwMissingPropertyError('config.fileExt');
    }
    if (!config.data) {
      _this.throwMissingPropertyError('config.data');
    }
  ...
  throwMissingPropertyError(property) {
