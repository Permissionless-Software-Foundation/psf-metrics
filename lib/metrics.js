...
class Metrics {
  ...
  async tokenOutflows (startBlock, stopBlock) {
    try {
      ...
      // Get transaction data for the 245 address of the TL app.
      ...
      for (let i = 0; i < sortTxData.length; i++) {
        ...
        try {
          ...
          if (
            ...
            const outputAmount = Number(opReturn.amounts[0]) / Math.pow(10, 8)
            const burnAmount = this.bchUtil.util.round8(
              inputTotal - outputAmount
            )
